...
---
## 6. FPGA Result and Comparison Summary

The FPGA implementation of the 64→8→10 MLP achieves a **fixed latency of 5 clock cycles**, which is expected for a fully pipelined hardware design. After the pipeline is filled, the FPGA can produce **one result per cycle**, giving extremely low per-sample latency.

Compared with the Phase 3 CUDA results:

- **FPGA** provides the **lowest latency** (5 cycles, deterministic).  
- **CUDA GPU** provides the **highest throughput** in large batches, achieving **over 120× speedup** relative to CPU.  
- **CPU, CUDA, and FPGA outputs all match**, confirming correctness across all implementations.

This demonstrates the performance trade-off across platforms:

- **FPGA excels at low latency**  
- **GPU excels at high throughput**  
- **CPU serves as the functional baseline**
